...
Description:
  Extracts the first audio stream from one or more MKV files and encodes it as
  MP3 using ffmpeg. By default, files are saved beside their sources using
  high-quality VBR.
...
Options:
  ...
  --delete-original, --DO Delete each MKV after its MP3 is safely written.
  -h, --help              Show this help message.
  --                      Treat all remaining arguments as file names.

Examples:
  1. Convert specific files (saves in the same folders):
     ./$script_name video1.mkv "video 2.mkv"

  2. Convert all MKVs in the current folder:
     ./$script_name *.mkv

  3. Convert files into an existing output folder:
     ./$script_name *.mkv ./converted_audio

  4. Create an output folder and encode every file at 320 kbps:
     ./$script_name --bitrate 320k --output-dir ./converted_audio *.mkv

  5. Convert files, then delete each successfully converted MKV:
     ./$script_name --delete-original *.mkv

Notes:
  The first audio stream and global metadata are used; video is omitted.
  Existing MP3s are skipped unless --force is specified. --delete-original
  only removes an MKV after a successful conversion; skipped and failed files
  are always retained.
...
for mkv_file in "${files_list[@]}"; do
    ...
    if [[ "$mkv_file" != *.mkv ]]; then
        printf "Ignored: '%s' is not an MKV file.\n" "$mkv_file" >&2
        failed=$((failed + 1))
        continue
    fi

    if [ -n "$dest_dir" ]; then
        base_name=$(basename "$mkv_file")
        output_file="$dest_dir/${base_name%.*}.mp3"
    else
        output_file="${mkv_file%.*}.mp3"
    fi
    ...
    # Encode into a temporary file, then move it into place. This prevents a
    ...
    if ffmpeg -hide_banner -loglevel error -stats -y \
        ...
        -id3v2_version 3 -f mp3 "$current_temp"; then
